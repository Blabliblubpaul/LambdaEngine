...
public ref struct  ReadonlyComponentEnumerator<T0> where T0 : unmanaged, IEcsComponent {
    private readonly ReadOnlyMemory<int>[] _ids;
    ...
    /// <summary>
    /// Returns a <see cref="ReadonlyComponentRef{T0}"/> for the index to which the enumerator is currently pointing.
    ...
    public ReadonlyComponentRef<T0> Current {
        get {
            ...
            if (IsValid && !_isAtEnd) {
                return new ReadonlyComponentRef<T0>(_ids[_arrayIndex].Span[_componentIndex], in _components0[_arrayIndex].Span[_componentIndex]);
            }
    ...
    internal  ReadonlyComponentEnumerator(EcsWorld world, NativeMemoryManager<int>[] ids, NativeMemoryManager<T0>[] components0) {
        _world = world;
        _version = _world._version;
        
        _ids = new ReadOnlyMemory<int>[ids.Length];
