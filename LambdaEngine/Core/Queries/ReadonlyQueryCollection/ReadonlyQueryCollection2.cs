...
public class ReadonlyQueryCollection<T0, T1> : IQueryCollection
    ...
    private ReadonlyQueryCollection(EcsWorld world, NativeMemoryManager<int>[] ids, NativeMemoryManager<T0>[] c0, NativeMemoryManager<T1>[] c1) {
        _world = world;
    ...
    internal class ReadonlyQueryCollectionBuilder {
        ...
        public ReadonlyQueryCollection<T0, T1> Build() {
            ...
            return new ReadonlyQueryCollection<T0, T1>(_world, ids.ToArray(), memory0.ToArray(), memory1.ToArray());
        }
    }
}