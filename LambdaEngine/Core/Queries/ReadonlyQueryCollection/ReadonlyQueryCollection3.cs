...
public class ReadonlyQueryCollection<T0, T1, T2> : IQueryCollection
    ...
    public ComponentEnumerable<T0, T1, T2> GetComponents() {
        if (!IsValid) {
            throw new InvalidOperationException("This collection is invalid.");
        }
        
        return new ComponentEnumerable<T0, T1, T2>(_world, _ids, _components0, _components1, _components2);
    }
    ...
    internal class ReadonlyQueryCollectionBuilder {
        ...
        public ReadonlyQueryCollectionBuilder(EcsWorld world) { _world = world; }
        ...
        public ReadonlyQueryCollection<T0, T1, T2> Build() {
            ...
            return new ReadonlyQueryCollection<T0, T1, T2>(_world, ids.ToArray(), memory0.ToArray(), memory1.ToArray(), memory2.ToArray());
        }
    }
}