...
public class ComponentEnumerable<T0, T1, T2, T3>
    ...
    public ComponentEnumerator<T0, T1, T2, T3> GetEnumerator() {
        if (!IsValid) {
            throw new InvalidOperationException("This collection is invalid.");
        }

        return new ComponentEnumerator<T0, T1, T2, T3>(_world, _ids, _components0, _components1, _components2, _components3);
    }
}