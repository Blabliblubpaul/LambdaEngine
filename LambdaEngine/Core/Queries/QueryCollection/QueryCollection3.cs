...
public class QueryCollection<T0, T1, T2> : IQueryCollection
    ...
    internal class QueryCollectionBuilder {
        ...
        public QueryCollection<T0, T1, T2> Build() {
            ...
            return new QueryCollection<T0, T1, T2>(_world, ids.ToArray(), memory0.ToArray(), memory1.ToArray(), memory2.ToArray());
        }
    }
}